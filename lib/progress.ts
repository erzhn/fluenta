...
export async function updateVocabWord(userId: string, wordId: string, box: number, intervalDays: number) {
  ...
  try {
    await supabase.from('vocabulary_progress').upsert(payload, { onConflict: 'user_id,word_id' })
  } catch {
...
export async function getVocabularyProgress(userId: string): Promise<Record<string, { box: number; intervalDays: number; dueDate: string; seen: boolean }>> {
  try {
    const { data } = await supabase
      ...
      .select('word_id,box,interval_days,due_date')
      .eq('user_id', userId)
    if (data) {
...
export async function logActivity(userId: string, minutes: number, xp: number) {
  ...
  try {
    ...
    const updatedXP = (existing?.xp ?? 0) + xp
    await supabase.from('activity_log').upsert(
      { user_id: userId, date: today, minutes: updatedMins, xp: updatedXP },
...
export async function getActivityLog(userId: string, days: number): Promise<Record<string, { minutes: number; xp: number }>> {
  try {
    ...
    const { data } = await supabase
      .from('activity_log')
      .select('date,minutes,xp')
      .eq('user_id', userId)
      .gte('date', since.toISOString().slice(0, 10))
    if (data) {
      return Object.fromEntries(data.map(r => [r.date, { minutes: r.minutes, xp: r.xp }]))
    }
  } catch { /* fall through */ }
...
export async function updateStreak(userId: string) {
  try {
    ...
    await supabase.from('profiles').update({ streak, last_active_date: today }).eq('user_id', userId)
  } catch {
  ...
  try {
    ...
    await Promise.all(
      Object.entries(vocabData).map(([wordId, v]) => {
        const intervalDays = intervals[Math.min(v.box, intervals.length - 1)]
        return updateVocabWord(userId, wordId, v.box, intervalDays)
      })
    )
