...
export function StreakCounter({ streak, hasActivityToday = true, compact = false }: StreakCounterProps) {
  if (compact) {
    return (
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-[hsl(var(--foreground-subtle))] text-xs">дн.</span>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col items-center text-center">
      ...
      <div className="text-[hsl(var(--foreground-muted))] text-sm mt-0.5">дней подряд</div>
      ...
      <AnimatePresence>
        ...
        {streak === 0 && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="mt-3 text-[hsl(var(--foreground-subtle))] text-xs"
          >
            Начни серию сегодня!
          </motion.div>
        )}
