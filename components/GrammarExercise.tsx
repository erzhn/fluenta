...
export function GrammarExercise({ topic, level, exercise, onNext, onCorrect }: GrammarExerciseProps) {
  ...
  const exerciseDisplay = exercise.exercise.replace(
    /___+/g,
    `<span class="inline-block min-w-[80px] border-b-2 border-primary text-center px-2 text-primary">${
  ...
  return (
    <div className="bg-white/[0.04] border border-white/10 rounded-2xl p-6">
      <p className="text-muted-foreground text-sm mb-4">{exercise.instruction}</p>
        ...
        <>
          <input
            ...
            className="w-full bg-white/[0.06] border border-white/10 rounded-xl px-4 py-3
              text-white placeholder:text-[#334155] outline-none focus:border-primary/50 transition-colors mb-3"
          />
          ...
          <button onClick={handleCheck} disabled={!userAnswer.trim() || checking}
            className="btn-glow w-full py-3 bg-primary hover:bg-[#5558e8] disabled:opacity-40
