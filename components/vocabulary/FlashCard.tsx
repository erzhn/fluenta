...
function HighlightedContext({ context, word }: { context: string; word: string }) {
  ...
  if (idx === -1) return <span className="italic text-muted-foreground">"{context}"</span>;
  return (
    <span className="italic text-muted-foreground">
      "{context.slice(0, idx)}
      <mark className="bg-primary/30 text-[#818CF8] rounded-sm not-italic px-0.5 mx-0.5">
  ...
  return (
    ...
    >
      ...
      >
        {/* ── Front ── */}
        <div
          className="absolute inset-0 bg-card border border-border rounded-2xl flex flex-col items-center justify-center p-8 text-center"
          style={{ backfaceVisibility: "hidden", WebkitBackfaceVisibility: "hidden" }}
        >
          <div className="flex items-center gap-2 mb-1">
            {isNew && <span className="text-[10px] font-bold bg-primary/20 text-[#818CF8] px-2 py-0.5 rounded-full">Новое</span>}
            {isMastered && <span className="text-[10px] font-bold bg-[#10B981]/20 text-[#10B981] px-2 py-0.5 rounded-full">Выучено</span>}
            {word.part_of_speech && (
              <span className="text-[10px] text-muted-foreground bg-background px-2 py-0.5 rounded-full">{word.part_of_speech}</span>
            )}
          </div>
          ...
          {word.phonetic && (
            <p className="text-muted-foreground font-mono text-base mb-4">{word.phonetic}</p>
          )}

          <button
            onClick={speak}
            className="p-2.5 rounded-xl bg-background hover:bg-secondary border border-border hover:border-[#475569] text-muted-foreground hover:text-white transition-all"
          >
        ...
        >
          ...
          {word.part_of_speech && (
            <span className="text-[10px] font-bold bg-secondary text-muted-foreground px-2.5 py-1 rounded-full uppercase tracking-wider">
              {word.part_of_speech}
            </span>
          )}
          ...
          <button
            onClick={speak}
            className="mt-4 p-2 rounded-xl bg-white/5 hover:bg-white/10 text-muted-foreground hover:text-white transition-all"
          >
