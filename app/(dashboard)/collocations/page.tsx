  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-5xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-[hsl(var(--foreground-muted))] text-sm mt-1">Слова которые «ходят вместе» · {COLLOCATIONS.length} коллокаций · {learned.size} изучено</p>
      </div>
          ...
          <div key={col.id}
            ...
            <div className="flex items-start justify-between mb-2">
              <div className="flex items-center gap-2">
                <span className="text-xs px-2 py-0.5 rounded-full bg-[hsl(var(--accent))]/10 text-[hsl(var(--accent))] font-mono font-bold">
                  {col.verb ?? col.adjective ?? '•'}
                </span>
                <span className="text-xs text-[hsl(var(--foreground-subtle))]">{col.level}</span>
              </div>
              <div className="flex gap-1.5">
                ...
                <button onClick={() => toggleLearn(col.id)}
                  className={`w-7 h-7 rounded-lg flex items-center justify-center text-xs transition-all ${
                    learned.has(col.id) ? 'bg-green-500/20 text-green-400' : 'bg-white/[0.06] text-[hsl(var(--foreground-muted))] hover:text-white'
                  }`}>
                  {learned.has(col.id) ? '✓' : '+'}
                </button>
              </div>
            ...
            <p className="text-[hsl(var(--accent))] text-sm font-medium mb-2">{col.translation}</p>
            <p className="text-[hsl(var(--foreground-muted))] text-sm italic mb-3">&quot;{col.example}&quot;</p>

            {col.wrongExample && (
              <button onClick={() => setShowWrong(s => { const n = new Set(s); n.has(col.id) ? n.delete(col.id) : n.add(col.id); return n })}
                className="text-xs text-[hsl(var(--foreground-subtle))] hover:text-[hsl(var(--foreground-muted))] transition-colors">
