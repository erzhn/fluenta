...
export default function ReadingSpeedPage() {
  ...
  return (
    <div className="p-4 sm:p-8 max-w-3xl mx-auto">
      ...
      <p className="text-[hsl(var(--foreground-muted))] mb-6">Читай текст и нажми &quot;Готово&quot; когда закончишь</p>
        ...
        <>
          <div className="bg-white/[0.04] border border-white/10 rounded-2xl p-6 mb-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-xs px-2 py-1 rounded-full bg-[hsl(var(--accent))]/10 text-[hsl(var(--accent))]">{text.level}</span>
            </div>
            <p className="text-[hsl(var(--foreground-muted))] text-sm">{text.wordCount} слов · Норма для {text.level}: {norm} слов/мин</p>
          </div>
            ...
            {TEXTS.map((t, i) => (
              <button key={i} onClick={() => setTextIdx(i)}
                className={`px-3 py-1.5 rounded-xl text-sm transition-all border ${
                  i === textIdx ? 'bg-[hsl(var(--accent))] border-[hsl(var(--accent))] text-white' : 'bg-white/[0.04] border-white/10 text-[hsl(var(--foreground-muted))]'
                }`}>
      ...
      {phase === 'reading' && (
        <>
          <div className="flex items-center justify-between mb-4">
            <span className="text-[hsl(var(--accent))] font-mono text-lg font-bold">{elapsed}с</span>
            <span className="text-[hsl(var(--foreground-muted))] text-sm">{text.wordCount} слов</span>
          </div>
      ...
      {phase === 'result' && (
        <>
          <div className="bg-white/[0.04] border border-white/10 rounded-2xl p-8 mb-6 text-center">
            <p className="text-[hsl(var(--foreground-muted))] text-sm mb-2">Твой результат</p>
            <p className="text-6xl font-bold mb-2"
              style={{ color: percentage >= 100 ? '#10b981' : percentage >= 70 ? '#f59e0b' : '#ef4444' }}>
              {wpm}
            </p>
            <p className="text-[hsl(var(--foreground-muted))] mb-6">слов в минуту</p>
            ...
            <div className="flex justify-between text-xs text-[hsl(var(--foreground-subtle))]">
              <span>0</span>
          ...
          <div className="flex gap-3">
            ...
            <button onClick={() => setPhase('intro')}
              className="px-5 py-3 bg-white/[0.06] hover:bg-white/10 text-[hsl(var(--foreground-muted))] rounded-xl transition-colors border border-white/10">
              Повторить
            </button>
          </div>
        </>
