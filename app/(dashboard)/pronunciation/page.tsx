      ...
      {LEVELS.map((l) => (
        <button key={l} onClick={() => onChange(l)}
          className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
            value === l ? 'bg-primary text-white' : 'text-muted-foreground hover:text-white bg-white/[0.03]'
          }`}>{l}
        </button>
      ))}
    </div>
  ...
  return (
    <div className={`${glass} rounded-2xl p-5 sm:p-6 space-y-5`}>
      <div className="flex items-center justify-between flex-wrap gap-3">
        <span className="text-muted-foreground text-xs uppercase tracking-wider">Уровень</span>
      ...
      <button onClick={next}
        className="w-full flex items-center justify-center gap-2 py-3 rounded-xl text-muted-foreground hover:text-white hover:bg-white/[0.04] transition-all text-sm font-medium border border-white/[0.06]">
  ...
  return (
    <div className={`${glass} rounded-2xl p-5 sm:p-6 space-y-5`}>
      ...
      <div className="bg-white/[0.03] rounded-xl p-4 border border-white/[0.06]">
        <p className="text-muted-foreground text-xs uppercase tracking-wider mb-2">Читай вслух:</p>
        {diff.length > 0 ? (
        ...
        {score !== null && (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-4">
            ...
            <button onClick={reset}
              className="w-full flex items-center justify-center gap-2 py-3 rounded-xl text-muted-foreground hover:text-white hover:bg-white/[0.04] transition-all text-sm border border-white/[0.06]">
  ...
  return (
    <div className={`${glass} rounded-2xl p-5 sm:p-6 space-y-5`}>
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <button onClick={newTopic} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-muted-foreground hover:text-white bg-white/[0.03] border border-white/[0.06] text-xs transition-all">
          <RefreshCw className="w-3 h-3" /> Новая тема
        </button>
      </div>
      <div className="bg-primary/10 border border-primary/30 rounded-xl p-4 text-center">
        <p className="text-[#818cf8] font-bold text-lg">{topic}</p>
        <p className="text-muted-foreground text-xs mt-1">Говори 30 секунд на английском</p>
      </div>
      {recording && (
      ...
      {done && transcript && (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
          <div className="bg-white/[0.03] rounded-xl p-4 border border-white/[0.06]">
            <p className="text-muted-foreground text-xs mb-1">Ты сказал:</p>
            <p className="text-white/80 text-sm">{transcript}</p>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-muted-foreground text-sm">Слов произнесено: <span className="text-white font-bold">{wordCount}</span></span>
  ...
  return (
    <div className={`${glass} rounded-2xl p-5 sm:p-6 space-y-5`}>
      <div className="text-center">
        <span className="text-muted-foreground text-xs bg-white/[0.04] px-3 py-1 rounded-full border border-white/[0.06]">
          Звук: {pair.sound}
        </span>
      </div>
          ...
          return (
            <button key={w} onClick={() => playWord(w)}
              className={`p-6 rounded-2xl border font-black text-2xl transition-all ${
                playing === w ? 'bg-primary/20 border-primary scale-[1.03] text-[#818cf8]' : 'bg-white/[0.03] border-white/10 text-white hover:border-primary/50'
              }`}>
              {word}
            </button>
          )
              ...
              <button key={w} onClick={() => makeGuess(w)}
                className={`py-4 rounded-xl font-bold border-2 transition-all text-lg ${
                  guess === w ? (correct ? 'bg-emerald-500/20 border-emerald-500 text-emerald-400' : 'bg-red-500/20 border-red-500 text-red-400')
                             : 'bg-white/[0.04] border-white/10 text-white hover:border-primary/60'
                }`}>
      ...
      {phase === 'result' && (
        <div className="space-y-4">
          ...
          <button onClick={next} className="w-full py-3 rounded-xl text-muted-foreground hover:text-white bg-white/[0.03] border border-white/[0.06] text-sm font-medium transition-all">
            Следующая пара →
          </button>
        </div>
      )}
      ...
      <div className="flex gap-2 justify-center">
        {TONGUE_TWISTERS.map((_, i) => (
          <button key={i} onClick={() => { setIdx(i); setScores({}) }}
            className={`w-2 h-2 rounded-full transition-all ${i === idx ? 'bg-primary scale-125' : 'bg-white/20'}`}/>
        ))}
      </div>
    ...
    <div className={`${glass} rounded-2xl p-5 space-y-4`}>
      ...
      {weakSounds.length > 0 && (
        <div className="space-y-2">
          <p className="text-muted-foreground text-xs uppercase tracking-wider">Слабые звуки</p>
          {weakSounds.map((x) => (
