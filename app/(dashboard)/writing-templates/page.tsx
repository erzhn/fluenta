...
export default function WritingTemplatesPage() {
  ...
  if (selected) return (
    <div className="p-4 sm:p-8 max-w-3xl mx-auto">
      ...
      <div className="flex items-center gap-2 mb-2">
        <span className="text-xs px-2 py-0.5 rounded-full bg-[hsl(var(--accent))]/10 text-[hsl(var(--accent))]">{selected.type}</span>
        <span className="text-[hsl(var(--foreground-subtle))] text-xs">{selected.level} · {selected.wordCount}</span>
      </div>
      <h1 className="text-2xl font-bold text-white mb-2">{selected.title}</h1>
      <p className="text-[hsl(var(--foreground-muted))] text-sm mb-6">{selected.description}</p>
      ...
      <div className="bg-[#f59e0b]/5 border border-[#f59e0b]/20 rounded-2xl p-4 mb-6">
        ...
        <ul className="space-y-1">
          {selected.tips.map((tip, i) => (
            <li key={i} className="text-[hsl(var(--foreground-muted))] text-sm flex gap-2">
      ...
      <div className="space-y-4 mb-6">
        {selected.structure.map((part, i) => (
          <div key={i} className="bg-white/[0.04] border border-white/10 rounded-2xl p-5">
            ...
            <p className="text-[hsl(var(--foreground-subtle))] text-xs mb-3 italic">{part.example}</p>
            <textarea
              ...
              className="w-full bg-white/[0.06] border border-white/10 rounded-xl px-4 py-3 text-white
                placeholder:text-[#334155] outline-none focus:border-[hsl(var(--accent))]/50 transition-colors text-sm resize-none"
            />
          </div>
        ))}
      </div>
      ...
      <button onClick={generateWithAI} disabled={generating}
        className="w-full py-4 bg-[hsl(var(--accent))] hover:bg-[#5558e8] disabled:opacity-50 text-white font-semibold rounded-2xl transition-colors mb-4">
        {generating ? '⏳ AI анализирует...' : '✨ Получить AI-обратную связь'}
      </button>

      {aiSuggestion && (
        <div className="bg-[hsl(var(--accent))]/5 border border-[hsl(var(--accent))]/20 rounded-2xl p-5">
          <p className="text-[hsl(var(--accent))] font-semibold text-sm mb-3">AI Обратная связь</p>
          <p className="text-[hsl(var(--foreground-muted))] text-sm whitespace-pre-wrap leading-relaxed">{aiSuggestion}</p>
        </div>
      )}
    </div>
  )
  ...
  return (
    <div className="p-4 sm:p-8 max-w-4xl mx-auto">
      ...
      <p className="text-[hsl(var(--foreground-muted))] text-sm mb-6">IELTS · деловые письма · жалобы · эссе</p>

      <div className="flex gap-2 mb-6">
        {types.map(t => (
          <button key={t} onClick={() => setFilter(t)}
            className={`px-4 py-2 rounded-xl text-sm font-medium transition-all border ${
              filter === t ? 'bg-[hsl(var(--accent))] border-[hsl(var(--accent))] text-white' : 'bg-white/[0.04] border-white/10 text-[hsl(var(--foreground-muted))] hover:text-white'
            }`}>{t}</button>
        ))}
      </div>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {filtered.map(t => (
          <button key={t.id} onClick={() => setSelected(t)}
            ...
            <div className="flex items-center gap-2 mb-2">
              <span className="text-xs px-2 py-0.5 rounded-full bg-[hsl(var(--accent))]/10 text-[hsl(var(--accent))]">{t.type}</span>
              <span className="text-[hsl(var(--foreground-subtle))] text-xs">{t.level}</span>
            </div>
            <p className="text-white font-semibold group-hover:text-[#a5b4fc] transition-colors">{t.title}</p>
            <p className="text-[hsl(var(--foreground-muted))] text-sm mt-1">{t.description}</p>
            <p className="text-[hsl(var(--foreground-subtle))] text-xs mt-2">{t.wordCount}</p>
          </button>
        ))}
      </div>
    </div>
