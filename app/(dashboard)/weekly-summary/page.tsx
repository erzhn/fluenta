      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
        {[
          ...
          <div key={s.label} className="bg-white/[0.04] border border-white/10 rounded-2xl p-4 text-center">
            ...
            <p className="text-[hsl(var(--foreground-muted))] text-xs mt-1">{s.label}</p>
          </div>
        ))}
      </div>
        ...
        <div className="flex items-end gap-3 h-32">
          {data.days.map(day => (
            <div key={day.date} className="flex-1 flex flex-col items-center gap-2">
              ...
              <span className="text-[hsl(var(--foreground-muted))] text-xs">{day.label}</span>
            </div>
          ))}
        </div>
      </div>

      <div className="bg-gradient-to-r from-[#6366f1]/10 to-[#8b5cf6]/10 border border-[hsl(var(--accent))]/20 rounded-2xl p-5">
