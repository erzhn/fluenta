            ...
            <div className="flex flex-col gap-2">
              {TOPICS[level].map(t => (
                <button key={t} onClick={() => setTopic(t)}
                  className={`text-left px-4 py-2.5 rounded-xl border text-sm transition-all ${topic === t ? 'border-primary bg-primary/15 text-white' : 'border-white/10 text-muted-foreground hover:text-white hover:border-white/20'}`}>
                  {t}
                </button>
              ))}
            </div>
          ...
          <div className="bg-white/[0.04] border border-white/10 rounded-2xl overflow-hidden">
            <div className="px-5 pt-4 pb-2">
              <p className="text-muted-foreground text-xs">
                ...
                <span className="ml-3 text-muted-foreground">· Минимум {minWords} слов</span>
              </p>
            </div>
            <textarea
              value={text}
            ...
            <div className="flex items-center justify-between px-5 py-3 border-t border-white/[0.06]">
              <div className="flex items-center gap-2">
                <span className={`text-sm font-medium ${enoughWords ? 'text-[#10b981]' : 'text-muted-foreground'}`}>
                  {words} слов
                </span>
                {!enoughWords && (
                  <span className="text-xs text-muted-foreground">/ нужно {minWords}</span>
                )}
              </div>
