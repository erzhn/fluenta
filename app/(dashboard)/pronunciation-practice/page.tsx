...
export default function PronunciationPracticePage() {
  ...
  async function generatePhrases() {
    const data = await generate<{phrases:string[]}>('pronunciation_phrases', aiTopic||'everyday situations', level)
    if (data?.phrases?.length) { setAiPhrases(data.phrases); setUsingAI(true); setPhraseIdx(0) }
  }
  ...
  return (
    <div className="p-4 sm:p-8 max-w-2xl mx-auto">
      ...
      <div className="bg-white/[0.04] border border-white/10 rounded-2xl p-6 sm:p-8 mb-6 text-center">
        <p className="text-muted-foreground text-xs uppercase tracking-wider mb-4">Повтори вслух:</p>
      ...
      {score !== null && (
        <div className="flex gap-3">
          <button onClick={() => { setHeard(''); setScore(null) }}
            className="flex-1 py-3 bg-white/[0.06] hover:bg-white/10 text-muted-foreground font-medium rounded-xl transition-colors border border-white/10">
            ↺ Повторить
          </button>
      ...
      {attempts.length > 0 && (
        <div className="mt-6">
          <p className="text-muted-foreground text-xs uppercase tracking-wider mb-3">История попыток</p>
          <div className="space-y-2">
            {attempts.slice(-5).reverse().map((a, i) => (
              <div key={i} className="flex items-center justify-between bg-white/[0.03] rounded-xl px-4 py-2.5">
                <span className="text-muted-foreground text-sm truncate flex-1">{a.phrase.slice(0, 40)}...</span>
