      ...
      <motion.div custom={5} variants={fadeUp} initial="hidden" animate="visible">
        <div className={`${glass} rounded-2xl p-5 sm:p-6`}>
          ...
          <div className="flex items-center justify-between text-[10px] text-muted-foreground mb-5">
            ...
            <span className="font-semibold text-primary">{xp.toLocaleString()} XP</span>
            {nextLvl && (
            ...
            {achievements.map((ach, i) => (
              <motion.div
                ...
                className={`rounded-2xl p-4 border transition-all relative overflow-hidden ${
                  ach.unlocked
                    ? 'bg-gradient-to-r from-[#6366f1]/10 to-[#8b5cf6]/10 border-primary/30'
              ...
              >
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-muted-foreground text-xs mt-0.5">{ach.desc}</p>
                  </div>
                  {ach.unlocked && (
              ...
              {recentConvs.map((conv) => (
                <div key={conv.id} className="flex items-center gap-4 px-5 py-3.5 hover:bg-white/[0.02] transition-colors group">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-muted-foreground text-xs mt-0.5">AI Conversation</p>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="text-muted-foreground text-xs">{relativeDate(conv.updated_at)}</p>
                    <p className="text-primary text-[10px] font-semibold mt-0.5">+20 XP</p>
                  </div>
                </div>
              ))}
            </div>
