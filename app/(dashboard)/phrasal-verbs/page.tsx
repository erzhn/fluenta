  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto">
      ...
      <div className="mb-6">
        ...
        <p className="text-[hsl(var(--foreground-muted))] text-sm">{total} фразовых глаголов по 5 темам</p>
      </div>
      ...
      <div className="flex gap-3 mb-6 flex-wrap">
        <div className="bg-white/[0.04] border border-white/10 rounded-xl px-4 py-3 flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[hsl(var(--foreground-muted))] text-xs">Изучено</p>
          </div>
        </div>
        <div className="bg-white/[0.04] border border-white/10 rounded-xl px-4 py-3 flex items-center gap-2">
          <span className="text-[hsl(var(--accent))] text-base">📚</span>
          <div>
            <p className="text-white font-semibold text-sm">{total - learned.size}</p>
            <p className="text-[hsl(var(--foreground-muted))] text-xs">Осталось</p>
          </div>
        </div>
        {learned.size > 0 && (
          <div className="bg-white/[0.04] border border-white/10 rounded-xl px-4 py-3 flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[hsl(var(--foreground-muted))] text-xs">Прогресс</p>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="relative mb-6">
        <span className="absolute left-4 top-1/2 -translate-y-1/2 text-[hsl(var(--foreground-muted))] text-sm">🔍</span>
        <input
          type="text"
          value={search}
          onChange={e => setSearch(e.target.value)}
          placeholder="Поиск: turn on, включать, lights..."
          className="w-full bg-white/[0.04] border border-white/10 rounded-xl pl-10 pr-4 py-3 text-white placeholder-[#475569] outline-none focus:border-[hsl(var(--accent))]/50 transition-colors text-sm"
        />
        {search && (
          <button onClick={() => setSearch('')}
            className="absolute right-4 top-1/2 -translate-y-1/2 text-[hsl(var(--foreground-muted))] hover:text-white transition-colors">
            ✕
          </button>
        )}
      </div>
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-sm font-medium transition-all border ${
              activeCategory === 'all'
                ...
                : 'bg-white/[0.04] border-white/10 text-[hsl(var(--foreground-muted))] hover:border-white/20 hover:text-white'
            }`}
          >
            Все ({total})
          </button>
      ...
      {!search && activeCategory !== 'all' && (
        <div className="mb-5 px-4 py-3 bg-[hsl(var(--accent))]/10 border border-[hsl(var(--accent))]/20 rounded-xl">
      ...
      {search && (
        <p className="text-[hsl(var(--foreground-muted))] text-sm mb-4">
          ...
          return (
            ...
            >
              ...
              <button
                ...
                className={`absolute top-3 right-3 w-7 h-7 rounded-full flex items-center justify-center text-sm transition-all ${
                  isLearned
                    ...
                    : 'bg-white/[0.06] text-[hsl(var(--foreground-subtle))] hover:text-white'
                }`}
              >
                {isLearned ? '✓' : '○'}
              </button>
              ...
              <div className="pr-8">
                ...
                <p className="text-[hsl(var(--accent))] text-sm">{verb.translation}</p>
              </div>
              ...
              {isExpanded && (
                <div className="mt-4 pt-4 border-t border-white/[0.08] space-y-2">
                  {verb.examples.map((ex, i) => (
                    <p key={i} className="text-[hsl(var(--foreground-muted))] text-sm italic">"{ex}"</p>
                  ))}
                  <div className="flex items-center gap-2 pt-1">
                    <button
                      onClick={e => { e.stopPropagation(); toggleFlip(verb.id) }}
                      className="text-xs px-3 py-1.5 rounded-lg bg-[hsl(var(--accent))]/20 border border-[hsl(var(--accent))]/30 text-[#a5b4fc] hover:bg-[hsl(var(--accent))]/30 transition-all"
                    >
                      {isFlipped ? 'Скрыть перевод' : 'Показать перевод'}
                    </button>
                    {isFlipped && (
                      <span className="text-[hsl(var(--accent))] text-sm font-medium">{verb.translation}</span>
                    )}
                  </div>
                </div>
              )}
              ...
              {!isExpanded && (
                <p className="text-[hsl(var(--foreground-subtle))] text-xs mt-3">
      ...
      {displayedVerbs.length === 0 && (
        <div className="text-center py-16">
          ...
          <p className="text-[hsl(var(--foreground-muted))] text-sm">Попробуй другой запрос</p>
        </div>
      )}
    </div>
  )
