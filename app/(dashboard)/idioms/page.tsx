    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[hsl(var(--foreground-muted))] text-sm mt-1">{IDIOMS.length} идиом · {learned.size} изучено</p>
        </div>
      </div>
      ...
      <input type="text" value={search} onChange={e => setSearch(e.target.value)}
        ...
        className="w-full bg-white/[0.06] border border-white/10 rounded-xl px-4 py-3 text-white
          placeholder:text-[#334155] outline-none focus:border-[hsl(var(--accent))]/50 transition-colors mb-4" />

      <div className="flex gap-2 flex-wrap mb-6">
        {['Все', ...IDIOM_CATEGORIES].map(cat => (
          <button key={cat} onClick={() => setCategory(cat)}
            className={`px-3 py-1.5 rounded-xl text-xs font-medium transition-all border ${
              category === cat ? 'bg-[hsl(var(--accent))] border-[hsl(var(--accent))] text-white' : 'bg-white/[0.04] border-white/10 text-[hsl(var(--foreground-muted))] hover:text-white'
            }`}>
            {cat}
          </button>
        ))}
          ...
          <div key={idiom.id}
            ...
            <div className="flex items-start justify-between mb-3">
              <div className="flex-1">
                <span className="text-xs text-[hsl(var(--foreground-subtle))]">{idiom.category} · {idiom.level}</span>
              </div>
              <button onClick={e => { e.stopPropagation(); toggleLearn(idiom.id) }}
                className={`w-7 h-7 rounded-lg flex items-center justify-center text-xs transition-all ${
                  learned.has(idiom.id) ? 'bg-green-500/20 text-green-400' : 'bg-white/[0.06] text-[hsl(var(--foreground-muted))] hover:text-white'
                }`}>
                {learned.has(idiom.id) ? '✓' : '+'}
              </button>
            </div>
            ...
            <div className="flex items-center gap-2 mb-2">
              ...
              <button onClick={e => { e.stopPropagation(); speak(idiom.idiom) }}
                className="text-[hsl(var(--foreground-subtle))] hover:text-[hsl(var(--foreground-muted))] transition-colors text-sm">🔊</button>
            </div>

            {flipped.has(idiom.id) ? (
              <div>
                <p className="text-[hsl(var(--accent))] font-medium text-sm mb-2">{idiom.meaning}</p>
                <p className="text-[hsl(var(--foreground-muted))] text-sm italic">&ldquo;{idiom.example}&rdquo;</p>
              </div>
            ) : (
              <p className="text-[hsl(var(--foreground-subtle))] text-xs">Нажми чтобы увидеть значение</p>
            )}
          </div>
        ))}

        {filtered.length === 0 && (
          <div className="col-span-full text-center py-12 text-[hsl(var(--foreground-subtle))]">
            Ничего не найдено
          </div>
        )}
      </div>
